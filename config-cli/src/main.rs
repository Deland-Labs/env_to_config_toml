...
fn merge_env_files(source: &str, out: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if toml_path.exists() {
        ...
        for line in reader.lines() {
            let line = line?;
            if line.trim() == "# GENERATED BY ENV_TO_CONFIG_TOML START" {
                found_start = true;
            }
            if line.trim() == "[env]" {
                if !found_start {
                    let pos = std::io::SeekFrom::Current(0);
                    let pos_value = writer.seek(pos)?;
                    writer.seek(std::io::SeekFrom::Start(pos_value - "[env]".len() as u64 + 1))?;
