...
fn main() {
    ...
    let matches = clap::Command::new("merge")
        ...
        .arg(
            clap::arg!(--"source-path" <PATH>)
                .required(true)
                .help("The source directory containing .env files")
    ...
    let source = matches.get_one::<String>("source-path").unwrap();
...
fn merge_env_files(source: &str, out_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
