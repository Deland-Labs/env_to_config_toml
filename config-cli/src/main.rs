        ...
        for env_path in env_paths {
            ...
            for (key, value) in env {
                ...
                if env_vars.contains_key(&key) {
                    return Err(MergeError::DuplicateKey(key, env_path.display().to_string()).into());
                }
...
mod tests {
    ...
    fn test_merge_env_files_duplicate() {
        ...
        assert_eq!(result.to_string(), MergeError::DuplicateKey("A".to_owned(), duplicate_path.display().to_string()).to_string());
    }
    ...
    fn test_merge_env_files_invalid_pattern() {
        ...
        let result = args.get_merge_bytes().err().unwrap();
        assert_eq!(result.to_string(), MergeError::NoFileFound(pattern.to_owned()).to_string());
    }
}